...
int64_t convertReduceScatter(sdy::ReduceScatterOp op, int64_t nextChannelId,
  ...
  for (mlir::Attribute dimAxesAttr : op.getReduceScatterAxes()) {
    if (auto dimAxes = mlir::dyn_cast<AxisRefListAttr>(dimAxesAttr)) {
      for (AxisRefAttr axis : dimAxes.getValue()) {
        unreducedAxes.push_back(axis);
      }
    }
  }
  llvm::sort(unreducedAxes, AxisRefAttr::getMeshComparator(mesh));
  ManualComputationOp manualComputation = createFullyManualComputation(
      op.getLoc(), op.getTensor(), op.getOutSharding(), mesh, rewriter,
...
class StablehloExportManualReductionCollectivesPass
 ...
 public:
  ...
  explicit StablehloExportManualReductionCollectivesPass(
      bool exportReduceScatter) {
    this->exportReduceScatter = exportReduceScatter;
  }
  ...
  Option<bool> exportReduceScatter{
      *this, "export-reduce-scatter",
      llvm::cl::desc("Whether to enable export of sdy.reduce_scatter ops."),
      llvm::cl::init(false)};
  ...
  void runOnOperation() final {
    ...
    moduleOp.walk([&](mlir::Operation* op) {
      if (auto allReduce = mlir::dyn_cast<sdy::AllReduceOp>(op)) {
        ...
        if (exportReduceScatter || inputHasUnreducedAxes(reduceScatter)) {
          nextChannelId =
